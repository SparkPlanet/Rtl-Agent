...
extern struct ne_fp_ffp_norm_mw33 ne_fp_ffp_norm_mw33;
...
int main()
{
    ...
    uint64_t status_in = (din_a >> (44)) & 0x7;

    __CPROVER_assume(status_in == 0 || status_in == 1 || status_in == 2 || status_in == 4);
    __CPROVER_assume(din_mode == 1  || din_mode == 2);

    ne_fp_ffp_norm_mw33.a = din_a;
    ne_fp_ffp_norm_mw33.mode = din_mode;
    set_inputs();

    uint64_t ref_z = c_ne_fp_ffp_norm_mw33(din_a, din_mode);

    assert(ne_fp_ffp_norm_mw33.z == ref_z);
}